...
class PLContext(Enum):
    ...
    def build_step(self, deser_dict: Dict) -> PipelineStep:
        ...
        """
        if self in (PLContext.SELECT, PLContext.WITH_COLUMNS, PLContext.FILTER):
...
class MakeStep:
    ...
    def make(deser_dict: Dict) -> PipelineStep:
        ...
        if ctx is None: 
            raise ValueError("There is no 'context' key in the dictionary. Input is not conformant.")
        else:
...
class SortStep():
    ...
    def __init__(self, by: str | pl.Expr | Sequence[str] | Sequence[pl.Expr], descending: bool | Sequence[bool]):
        ...
        if len(self.by) != len(self.descending):
            raise ValueError(f"Input `by` (len {len(by)}) doesn't match the length of `descending` (len {len(self.descending)}).")
...
class GroupByAggStep():
    ...
    def from_partial_dict(d: dict) -> "GroupByAggStep":
        ...
        return GroupByAggStep(by = by, agg = agg)


    def apply_df(self, df: pl.LazyFrame | pl.DataFrame) -> pl.LazyFrame | pl.DataFrame:
        return df.group_by(self.by).agg(self.agg)
...
class GroupByDynAggStep():
    ...
    __slots__ = ("index_column", "every", "agg", "by", "period", "offset", "include_boundaries", "closed", "label", "start_by", "context")

    def __init__(self, 
        index_column: str
        , every: str
        , agg: List[pl.Expr]
        , by: str | pl.Expr | Sequence[str] | Sequence[pl.Expr] | None = None
        ...
        if closed not in ['left', 'right', 'both', 'none']:
            raise ValueError("Input `closed` must be one of ['left', 'right', 'both', 'none']. See polars's group_by_dynamic for more info.")
        
        if label not in ['left', 'right', 'datapoint']:
            raise ValueError("Input `closed` must be one of ['left', 'right', 'datapoint']. See polars's group_by_dynamic for more info.")
        
        if start_by not in ['window', 'datapoint', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']:
            raise ValueError("Input `closed` must be one of ['window', 'datapoint', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']. See polars's group_by_dynamic for more info.")
        
        # Because of a strange bug.
        if isinstance(index_column, pl.Expr):
            raise ValueError("Input `index_column` must be a string (name of a column) and not a Polars expression.")
        ...
        self.start_by: Literal['window', 'datapoint', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] = start_by

        if isinstance(by, (str, pl.Expr)):
            self.by: List[pl.Expr] | None = [to_expr(by)]
        elif isinstance(by, list):
            self.by: List[pl.Expr] | None = [to_expr(e) for e in by]
        elif by is None: 
            self.by: List[pl.Expr] | None = None
        else:
            raise ValueError("Input `by` can only be scalar str / pl.Expr or a list of them or None.")
    ...
    def from_partial_dict(d: Dict) -> "GroupByDynAggStep":
        return GroupByDynAggStep(
            ...
            , label=d['label']
            , start_by=d['start_by']
            , by=None if d["by"] is None else [pl.Expr.deserialize(StringIO(e), format="json") for e in d["by"]] 
            , agg=[pl.Expr.deserialize(StringIO(e), format="json") for e in d["agg"]]
        )
    ...
    def to_json(self) -> str:
        d = self.context.get_context_dict() | {
            'by': None if self.by is None else [e.meta.serialize(format="json") for e in self.by]
            , 'agg': [e.meta.serialize(format="json") for e in self.agg]
            , 'index_column': self.index_column
            , 'every': self.every
            , 'period': self.period
...
class ExprStep():
    ...
    def __init__(
        ...
        if isinstance(exprs, pl.Expr):
            ...
            self.exprs: List[pl.Expr] = [pl.Expr.deserialize(StringIO(exprs), format="json")]
        elif isinstance(exprs, list):
            self.exprs: List[pl.Expr] = list(exprs)

use pyo3_polars::{derive::polars_expr, export::polars_core::{with_match_physical_integer_type, utils::arrow::{types::NativeType, array::PrimitiveArray}}};
use polars::prelude::*;
use core::hash::Hash;
use DataType::*;
...
fn pl_list_jaccard(inputs: &[Series]) -> PolarsResult<Series>{
    ...
    if s1.inner_dtype().is_integer() {
        ...
        Ok(
            s1.into_iter().zip(s2.into_iter()).map(
                |(c1, c2)| {
                    match (c1, c2) {
                        (Some(c1), Some(c2)) => {

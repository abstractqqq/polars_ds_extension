...
use pyo3_polars::{
    ...
    export::polars_core::{POOL, utils::rayon::{
...
#[polars_expr(output_type=UInt32)]
fn pl_subseq_sim_cnt_zl2(inputs: &[Series], context: CallerContext, kwargs: SubseqQueryKwargs) -> PolarsResult<Series> {
    ...
    let n = if par {
        ...
        splits.into_par_iter().map(|(offset, len)| {
            let mut acc: u32 = 0;
            for (i, w) in windows[offset..offset+len].iter().enumerate() {
                let actual_i = i + offset;
                let normalized = w.iter().map(|x| (x - rolling_mean[actual_i]) / rolling_var[actual_i].sqrt()).collect::<Vec<_>>();

...
use polars::{series::{Series, IsSorted}, prelude::{PolarsResult, PolarsError}};
...
pub fn trapz(y:ArrayView1<f64>, x:ArrayView1<f64>) -> f64 {
    ...
    0.5 * (x_d.into_iter().zip(y_s.into_iter()).fold(0., |acc, (x,y)| 
        acc + x * y
    ))
...
fn pl_trapz(inputs: &[Series]) -> PolarsResult<Series> {
    ...
    if x.len() == 1 {
        ...
        Ok(
            Series::from_iter([trapz_dx(y, dx)])
        )
    } else if x.len() == y.len() {

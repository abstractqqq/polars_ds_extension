...
use faer::{FaerMat, IntoFaer, IntoNdarray, Row};
use kdtree::KdTree;
...
use ndarray::{aview1, indices, s, Array2, ArrayView1, ArrayView2, Axis};
...
fn pl_matrix_profile2(
    ...
    use faer::modules::core::mul::triangular::{matmul, BlockStructure};
    use faer::{Parallelism, Mat};


    let mat = Array2::from_shape_vec((output_size, m), points).unwrap();
    let a = mat.view().into_faer();
    let at = a.transpose();

    let mut acc = Mat::<f64>::from_fn(output_size, output_size, || f64::MIN);
    // zeros(output_size, output_size);
    matmul(
        acc.as_mut(),
    ...
    let res = acc.as_ref().into_ndarray();
    let res_t = res.t();
    ...
    for i in 0..output_size {
        ...
        let right_iter = res.row(i).slice(1..).iter();
        let left_iter = res_t.row(i).slice(0..i+1).iter();
        let it = (left_iter.chain(right_iter)).enumerate();

        let (amax, max) = it.fold((0usize, f64::MIN), |(amax, max), (j, x)| {
            if i.abs_diff(j) > exclusion {
                if x > amax {
                    (j, x)
                } else {
        ...
        dist_builder.append_value(2.0 * (mf64 - max));
        idx_builder.append_value(amax as u32);
    }
    
    let ca_dist = dist_builder.finish();
    let ca_id = idx_builder.finish();
    let s_dist = ca_dist.into_series();
    let s_id = ca_id.into_series();
    let out = StructChunked::new("profile", &[s_dist, s_id])?;
    Ok(out.into_series())

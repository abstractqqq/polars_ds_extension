...
fn pl_kaplan_meier(inputs: &[Series]) -> PolarsResult<Series> {
    ...
    let table = df
        .lazy()
        .group_by(["time_exit"])
        .agg([len().alias("cnt"), col("status").sum().alias("events")])
        .sort(["time_exit"], SortMultipleOptions::default())
        .with_column(
            (lit(n1 as u32) - col("cnt").cum_sum(false).shift_and_fill(1, lit(0)))
                .alias("n_at_risk"),
        )
        .select([
            col("time_exit").alias("t"),
            (lit(1f64)
                - col("events").cast(DataType::Float64) / col("n_at_risk").cast(DataType::Float64))
            .cum_prod(false)
            .alias("prob"),
        ])
        .collect()?;

...
fn pl_overlap_coeff(inputs: &[Series], context: CallerContext) -> PolarsResult<Series> {
    ...
    if ca2.len() == 1 {
        ...
        let out: Float64Chunked = if can_parallel {
            ...
            ca1.apply_nonnull_values_generic(DataType::Float64, |s| overlap_coeff(s, r, n))
        };
        Ok(out.into_series())
                ...
                let chunks: Vec<_> = splits
                    ...
                    .map(|(offset, len)| {
                        ...
                        let out: Float64Chunked =
                            binary_elementwise_values(&s1, &s2, |x, y| overlap_coeff(x, y, n));
                        out.downcast_iter().cloned().collect::<Vec<_>>()
                    })
                    .collect();
                Float64Chunked::from_chunk_iter(ca1.name(), chunks.into_iter().flatten())

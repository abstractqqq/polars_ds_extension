...
impl<'a> Gradient for LogRegProblem<'a> {
    ...
    fn gradient(&self, w: &Self::Param) -> Result<Self::Gradient, Error> {
        ...
        diff.col_as_slice_mut(0)
            .iter_mut()
            .for_each(|v| *v = stable_sigmoid(*v));
...
pub fn faer_logistic_reg(
    ...
    let mut solver = LBFGS::new(linesearch, 10)
        .with_tolerance_grad(EPSILON.sqrt().max(tol))
        .unwrap();
    ...
    let result = Executor::new(problem, solver)
        .configure(|state| state.param(w).max_iters(max_iters as u64))
        .run();

    match result {
        Ok(opt) => match opt.state.best_param {
            Some(best) => best,

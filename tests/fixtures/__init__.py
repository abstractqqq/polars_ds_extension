...
_MULTICHUNK = {
    "medium_multichunk": [
        f"medium_multichunk_p{i}.parquet" for i in range(4)
    ],
}
...
def load(name: str) -> pl.DataFrame:
    ...
    if name not in _ALL:
        raise ValueError(
            f"Unknown fixture {name!r}. Available: {sorted(_ALL)}"
        )

    if name in _MULTICHUNK:
        parts = [
            pl.read_parquet(_FIXTURES_DIR / fname)
            for fname in _MULTICHUNK[name]
        ]

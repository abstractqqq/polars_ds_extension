...
def test_kaplan_meier():
    ...
    df_result = pl.from_dict({"t": time, "prob": prob_surv})

    df = pl.from_dict({"status": y["Status"], "survival_time": y["Survival_in_days"]})

    df_pds_result = df.select(
        pds.query_kaplan_meier_prob("status", "survival_time").alias("estimate")
    ).unnest("estimate")
